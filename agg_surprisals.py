...
def agg_surprisals(surprisals, sentence_tokens, unks, in_data, model):
...
def main(args):
    ...
    sentence_tokens = utils.tokenize_file(args.sentences, args.model)
    unks = utils.unkify_file(args.sentences, args.model)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', '-model', '--m', '-m', type=str,
                        ...
                        help='model for cpllab Docker container. '
                             'if None, then split tokens by spaces and treat'
                             'all tokens as in-vocabulary (no UNKs)')
